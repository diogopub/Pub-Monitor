/**
 * Configurações — Gestão de Projetos e Acesso
 */
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground max-w-md">
              Use este botão se os dados na nuvem estiverem vazios e você queira enviar seu estado local atual.
            </p>
          </div>
          <Button 
            variant="outline" 
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-muted/50">
                ...
                <th className="p-3 text-right border-b border-border">Ações</th>
              </tr>
            </thead>
            <tbody>
              {authorizedUsers.map((u) => (
                <tr key={u.email} className="border-b border-border/50 hover:bg-muted/20">
                  ...
                  <td className="p-3 text-right">
                    {u.email !== user?.email && (
                      <button onClick={() => removeAuthorizedUser(u.email)} className="text-muted-foreground hover:text-red-500">
  ...
  return (
    <section className="p-4 sm:p-6 pt-0 border-t border-border mt-8">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h2 className="text-lg font-bold font-heading tracking-wide">Backup em Nuvem</h2>
      </div>
      <div className="max-w-2xl bg-card/40 backdrop-blur-sm border border-border rounded-xl p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div className="space-y-0.5">
            <h3 className="text-sm font-bold">Backup Automático</h3>
            <p className="text-xs text-muted-foreground">Drive (12:00 e 20:00).</p>
          </div>
          <Switch checked={autoBackupEnabled} onCheckedChange={(v) => updateSettings({ autoBackupEnabled: v })} />
        </div>
        <div className="space-y-2">
          <label className="text-xs font-bold text-muted-foreground uppercase">Script URL</label>
          <Input 
